  ...
  return <main className="min-h-screen bg-[#faf9f5] text-[#1f3529]">
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <Tabs value={view} onValueChange={setView}>
        ...
        {view!=="history"&&<div className="mb-7 flex flex-col gap-2 sm:flex-row sm:items-center"><span className="mr-1 text-sm font-semibold text-[#5f6e64]">Planning week</span>{([0,1] as const).map(offset=><button key={offset} type="button" onClick={()=>setWeekOffset(offset)} className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left transition sm:min-w-56 ${weekOffset===offset?"border-[#315d43] bg-[#315d43] text-white shadow-sm":"border-[#d9d5cc] bg-white text-[#315d43] hover:border-[#8aa293] hover:bg-[#f2f5f1]"}`}><span><span className="block text-xs font-bold uppercase tracking-wide opacity-75">{offset===0?"This week":"Next week"}</span><span className="mt-0.5 block text-sm font-semibold">{weekRange(offset)}</span></span>{weekOffset===offset&&<Check size={18}/>}</button>)}</div>}
