...
export default function Home() {
  ...
  const loadSharedRecipes=async()=>{try{const response=await fetch("/api/recipes");if(!response.ok)return;const {recipes:shared}=await response.json();if(Array.isArray(shared)&&shared.length>0){const existing=new Set(recipes.map(r=>r.id));const newShared=shared.filter(r=>!existing.has(r.id)).map((r:any)=>({...r,ingredients:Array.isArray(r.ingredients)?r.ingredients.map((i:any)=>typeof i==="string"?parseIngredientLine(i):i):[],directions:Array.isArray(r.directions)?r.directions.filter((d:any):d is string=>typeof d==="string"):[]}));if(newShared.length>0)setRecipes(current=>[...newShared,...current])}}catch(e){console.error("Failed to load shared recipes:",e)}};
