...
export default function Home() {
  ...
  const openRecipe=(recipe:Recipe)=>{setActiveRecipe(recipe);setIngredientsCollapsed(false);setCompletedSteps([]);setShowCompletedSteps(false);window.scrollTo({top:0,behavior:"smooth"})};
  ...
  return <main className="min-h-screen bg-[#faf9f5] text-[#1f3529]" style={{scrollbarGutter:'stable'}}>
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      {activeRecipe ? <article>
        ...
        <div className="overflow-hidden rounded-[1.75rem] border border-[#dedbd2] bg-white">
          ...
          <div className="p-6 sm:p-9 lg:p-12">
            ...
            <div className="mt-8 flex max-w-md items-center justify-between rounded-2xl bg-[#f2ecdf] p-3"><span className="font-medium">Cooking for</span><div className="flex items-center gap-1"><Button size="icon" variant="ghost" className="size-11 rounded-full sm:size-8" onClick={()=>changeServings(activeRecipe.id,-1)}><Minus size={17} className="sm:hidden"/><Minus size={15} className="hidden sm:block"/></Button><strong className="min-w-20 text-center">{servings[activeRecipe.id]||4} people</strong><Button size="icon" variant="ghost" className="size-11 rounded-full sm:size-8" onClick={()=>changeServings(activeRecipe.id,1)}><Plus size={17} className="sm:hidden"/><Plus size={15} className="hidden sm:block"/></Button></div></div>
            <div className="mt-10 grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] lg:gap-14">
              <section className="min-w-0"><button onClick={()=>setIngredientsCollapsed(v=>!v)} aria-expanded={!ingredientsCollapsed} aria-controls="ingredients-list" className="flex w-full items-center justify-between gap-3 text-left"><h3 className="font-serif text-2xl font-bold">Ingredients</h3><span className="flex shrink-0 items-center gap-1 rounded-lg border border-[#d8d5cd] bg-white px-3 py-2 text-sm font-semibold text-[#45644e] hover:bg-[#f1f4ef]">{ingredientsCollapsed?"Show":"Hide"}</span></button>{!ingredientsCollapsed&&<div id="ingredients-list" className="mt-4 divide-y divide-[#e8e0d1]">{(Array.isArray(activeRecipe.ingredients)?activeRecipe.ingredients:[]).map((raw,index)=>{const item=normalizeIngredient(raw);if(!item)return null;const amount=item.amount*(servings[activeRecipe.id]||4)/(activeRecipe.serves||4);return <div key={index} className="grid min-w-0 grid-cols-[max-content_minmax(0,1fr)] items-start gap-x-4 py-3"><strong className="whitespace-nowrap text-[#45644e]">{Math.round(amount*100)/100} {item.unit}</strong><span className="min-w-0 leading-6">{item.name}</span></div>})}</div>}</section>
              <section className="min-w-0"><h3 className="font-serif text-2xl font-bold">Directions</h3>{activeRecipe.directions?.length?<>
                <p className="mt-2 text-sm text-[#6d786f]">Tap a step when you finish it — completed steps tuck away so you can focus on what's next.</p>
                <ol className="mt-5 space-y-4">{activeRecipe.directions.map((step,index)=>completedSteps.includes(index)?null:<li key={index}><button onClick={()=>toggleStepDone(index)} className="flex w-full min-w-0 items-start gap-4 rounded-2xl border border-transparent p-2 text-left transition hover:border-[#dbe4dc] hover:bg-[#f5f8f4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315d43]/35"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#45644e] font-semibold text-white">{index+1}</span><p className="min-w-0 flex-1 pt-1 leading-7">{step}</p></button></li>)}</ol>
                ...
                {completedSteps.length>0&&<div className="mt-6"><button onClick={()=>setShowCompletedSteps(v=>!v)} className="text-sm font-semibold text-[#45644e] underline decoration-[#45644e]/35 underline-offset-2 hover:decoration-[#45644e]">{showCompletedSteps?"Hide":"Show"} {completedSteps.length} completed step{completedSteps.length===1?"":"s"}</button>{showCompletedSteps&&<ol className="mt-4 space-y-3">{activeRecipe.directions.map((step,index)=>!completedSteps.includes(index)?null:<li key={index}><button onClick={()=>toggleStepDone(index)} className="flex w-full min-w-0 items-start gap-4 rounded-2xl p-2 text-left text-[#8a9187] transition hover:bg-[#f5f8f4]"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#c7d3c8] font-semibold text-white"><Check size={16}/></span><p className="min-w-0 flex-1 pt-1 leading-7 line-through">{step}</p></button></li>)}</ol>}</div>}
      ...
      <header className="relative mb-8 min-h-[260px] overflow-hidden rounded-[1.75rem] sm:min-h-[350px]">
        ...
        <div className="relative flex min-h-[260px] max-w-2xl flex-col justify-center px-4 py-6 text-white sm:min-h-[350px] sm:px-10 sm:py-7 lg:px-12"><div className="w-full max-w-full rounded-2xl bg-[#13271d]/75 py-5 pl-5 pr-6 shadow-sm backdrop-blur-sm sm:w-fit sm:py-7 sm:pl-7 sm:pr-12">
          <p className="mb-3 text-xs font-bold uppercase tracking-[.2em] text-[#d5e7d8]">Cameron Family Table</p>
          <h2 className="font-serif text-2xl font-medium leading-tight sm:whitespace-nowrap sm:text-4xl lg:text-5xl">Good food, happy family.</h2>
                        ...
                        <section key={cat} className="min-w-0 w-full overflow-hidden rounded-2xl border border-[#ddd4c3] bg-[#fffdf8] shadow-[0_2px_10px_rgba(45,61,50,.04)]">
                          <div className="flex items-center justify-between border-b border-[#ebe6dc] bg-[#f6f1e7] px-4 py-3">
                            ...
                            <div className="flex items-center gap-2">
                              <button onClick={(e)=>{e.stopPropagation();setCollapsed(prev=>({...prev,[cat]:!prev[cat]}))}} aria-expanded={!collapsed[cat]} aria-controls={`cat-${cat}`} className="min-h-11 rounded-md border border-transparent bg-white/0 px-3 py-2 text-sm text-[#45644e] hover:bg-white/80 sm:min-h-0 sm:px-2 sm:py-1">{collapsed[cat]?"Expand":"Collapse"}</button>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex items-stretch border-t border-[#dedbd2] bg-white pb-[env(safe-area-inset-bottom)] shadow-[0_-2px_10px_rgba(45,61,50,.06)] sm:hidden">
        {nav.map(({value,label,icon:Icon})=><button key={value} onClick={()=>setView(value)} aria-label={label} aria-current={view===value?"page":undefined} className={`relative flex min-h-16 flex-1 flex-col items-center justify-center gap-1 text-xs font-semibold transition ${view===value?"text-[#244832]":"text-[#8a9187]"}`}><Icon size={22}/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className="absolute right-1/4 top-1.5 grid size-4 place-items-center rounded-full bg-[#315d43] text-[9px] leading-none text-white">{selected.length}</b>}</button>)}
      </nav>
      </>}
    </div>
