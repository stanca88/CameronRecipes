      ...
      <Tabs value={view} onValueChange={setView}>
        <div className="mb-4 flex items-center justify-between gap-2 border-b border-[#dedbd2] pb-4 pt-4 sm:pt-3">
          <div className="flex min-w-0 shrink items-center gap-2">
            ...
            <Button type="button" variant="ghost" aria-label="View history" onClick={()=>setView("history")} className={`size-11 shrink-0 rounded-lg p-0 shadow-none transition sm:size-9 ${view==="history"?"bg-[#315d43] text-white hover:bg-[#274d37]":"text-[#45644e] hover:bg-[#f2f5f1]"}`}><HistoryIcon size={18}/></Button>
          </div>
          <Button type="button" onClick={()=>setView("recipes")} className={`h-11 shrink-0 rounded-lg px-4 text-sm font-semibold shadow-none transition sm:h-9 ${view==="recipes"?"bg-[#315d43] text-white hover:bg-[#274d37]":"border border-[#d9d5cc] bg-white text-[#244832] hover:bg-[#f2f5f1]"}`}><BookOpen size={18}/>Recipes</Button>
        </div>
