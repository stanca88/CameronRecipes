      ...
      {view!=="recipes"&&<header className="relative -mx-4 -mt-6 mb-3 overflow-hidden rounded-none sm:mx-0 sm:mt-0 sm:rounded-[1.75rem]">
        ...
        <div className="relative flex items-stretch gap-2 px-5 py-3 sm:px-8">
          ...
          <TabsList className="flex h-11 flex-1 items-stretch gap-1 overflow-hidden rounded-lg border-0 bg-white/15 p-0 shadow-none backdrop-blur-sm sm:flex-none">
            {nav.map(({value,label,icon:Icon})=><TabsTrigger key={value} value={value} className="relative flex h-full flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-none border-0 bg-transparent px-2 text-xs font-semibold text-white/80 shadow-none transition first:rounded-l-md last:rounded-r-md hover:bg-white/10 hover:text-white data-[state=active]:bg-[#257F4B] data-[state=active]:text-white data-[state=active]:shadow-none sm:px-5 sm:text-sm"><Icon size={16} className="shrink-0"/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className="grid size-[30px] shrink-0 place-items-center rounded-full bg-white/20 text-xs font-bold leading-none text-white">{selected.length}</b>}</TabsTrigger>)}
          </TabsList>
        </div>
      </header>}
        {view==="recipes"
