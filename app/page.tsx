...
type WeeklyPlan = { selected: string[]; servings: Record<string,number>; checked: string[]; chefs: Record<string,string> };
type SavedWeek = { id: string; label: string; savedAt: string; meals: { id: string; title: string; emoji: string; people: number; chef?: string }[] };
...
export default function Home() {
  ...
  const [plans,setPlans]=useState<Record<string,WeeklyPlan>>({[weekKey(0)]:{selected:["pasta","chicken"],servings:{pasta:4,chicken:4},checked:[],chefs:{}}});
  ...
  const activePlan=plans[activeWeekKey]||{selected:[],servings:{},checked:[],chefs:{}};
  ...
  const setSelected=(update:(current:string[])=>string[])=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[],chefs:{}};return{...all,[activeWeekKey]:{...plan,selected:update(plan.selected)}}});
  const setServings=(update:(current:Record<string,number>)=>Record<string,number>)=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[],chefs:{}};return{...all,[activeWeekKey]:{...plan,servings:update(plan.servings)}}});
  const setChecked=(update:(current:string[])=>string[])=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[],chefs:{}};return{...all,[activeWeekKey]:{...plan,checked:update(plan.checked)}}});
  const setChef=(id:string,name:string)=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[],chefs:{}};return{...all,[activeWeekKey]:{...plan,chefs:{...(plan.chefs||{}),[id]:name}}}});
  const weekLabel=weekRange(weekOffset);

  useEffect(()=>{try{const raw=localStorage.getItem("cameron-family-table");if(raw){const s=JSON.parse(raw);setRecipes(s.recipes||starterRecipes);setHistory(s.history||[]);setPlans(s.plans||{[weekKey(0)]:{selected:s.selected||[],servings:s.servings||Object.fromEntries((s.selected||[]).map((id:string)=>[id,4])),checked:s.checked||[],chefs:{}}})}}finally{setLoaded(true)}},[]);
  ...
  const saveWeek=()=>{const meals=recipes.filter(r=>selected.includes(r.id)).map(r=>({id:r.id,title:r.title,emoji:r.emoji,people:servings[r.id]||4,chef:chefs[r.id]?.trim()||undefined}));if(!meals.length)return;setHistory(v=>[{id:crypto.randomUUID(),label:weekLabel,savedAt:new Date().toISOString(),meals},...v.filter(w=>w.label!==weekLabel)]);setView("history")};
  const deleteRecipe=(id:string)=>{setRecipes(v=>v.filter(recipe=>recipe.id!==id));setPlans(all=>Object.fromEntries(Object.entries(all).map(([key,plan])=>{const nextServings={...plan.servings};const nextChefs={...(plan.chefs||{})};delete nextServings[id];delete nextChefs[id];return[key,{...plan,selected:plan.selected.filter(recipeId=>recipeId!==id),servings:nextServings,chefs:nextChefs}]})));setActiveRecipe(null)};
