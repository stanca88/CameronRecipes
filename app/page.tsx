...
export default function Home() {
  ...
  const changeView=(v:string)=>{if(v!=="recipes")setPreRecipesView(v);setView(v)};
  const goToRecipes=()=>{setPreRecipesView(view);setView("recipes")};
        ...
        <div className="overflow-hidden rounded-[1.75rem] border border-[#dedbd2] bg-white">
          ...
          <div className="p-6 sm:p-9 lg:p-12">
            <div className="border-b border-[#e8e3da] pb-8">
              <p className="mb-2 text-xs font-bold uppercase tracking-[.2em] text-[#477457]">Family recipe</p><h2 className="max-w-3xl font-serif text-3xl font-bold leading-tight sm:text-5xl">{activeRecipe.title}</h2>
              ...
              {activeRecipe.sourceUrl&&<p className="mt-4 text-sm text-[#6d786f]"><a href={activeRecipe.sourceUrl} target="_blank" rel="noreferrer" className="font-semibold text-[#45644e] underline decoration-[#45644e]/35 underline-offset-2 hover:decoration-[#45644e]">{activeRecipe.sourceName||"View original recipe"}</a></p>}
            </div>
      ...
      <Tabs value={view} onValueChange={changeView}>
        <div className="mb-4 flex items-center justify-between gap-2 border-b border-[#dedbd2] pb-4 pt-4 sm:pt-3">
          <div className="flex min-w-0 shrink items-center gap-2">
            ...
            <Button type="button" variant="ghost" aria-label="View history" onClick={()=>changeView("history")} className={`size-11 shrink-0 rounded-lg p-0 shadow-none transition sm:size-9 ${view==="history"?"bg-[#315d43] text-white hover:bg-[#274d37]":"text-[#45644e] hover:bg-[#f2f5f1]"}`}><HistoryIcon size={18}/></Button>
          </div>
          <Button type="button" onClick={()=>{if(view==="recipes"){setView(preRecipesView)}else{goToRecipes()}}} className={`h-11 shrink-0 rounded-lg px-4 text-sm font-semibold shadow-none transition sm:h-9 ${view==="recipes"?"bg-[#315d43] text-white hover:bg-[#274d37]":"border border-[#d9d5cc] bg-white text-[#244832] hover:bg-[#f2f5f1]"}`}>{view==="recipes"?<><ArrowLeft size={18}/>Back</>:<><BookOpen size={18}/>Recipes</>}</Button>
        </div>

        {view!=="recipes"&&<div className="sticky top-0 z-30 mb-4 bg-[#faf9f5] pb-1.5 sm:static sm:mb-0 sm:bg-transparent sm:pb-0">
