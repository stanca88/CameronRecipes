      ...
      <Tabs value={view} onValueChange={setView}>
        <div className="mb-8 flex w-full items-end gap-1 border-b border-[#dedbd2]">
          <TabsList className="flex h-auto shrink-0 items-center justify-start gap-1 rounded-none bg-transparent p-0 shadow-none">
            {nav.map(({value,label,icon:Icon})=><TabsTrigger key={value} value={value} className="relative flex min-h-12 flex-1 items-center justify-center gap-2 rounded-none border-0 border-b-2 border-transparent bg-transparent px-3 py-3.5 text-sm font-semibold text-[#68716a] shadow-none transition hover:bg-transparent hover:text-[#315d43] data-[state=active]:border-[#315d43] data-[state=active]:bg-transparent data-[state=active]:text-[#244832] data-[state=active]:shadow-none sm:min-h-11 sm:flex-none sm:px-5 sm:py-3"><Icon size={20} className="shrink-0"/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className="rounded-full bg-[#315d43] px-1.5 py-0.5 text-[10px] leading-none text-white sm:px-2 sm:text-xs">{selected.length}</b>}</TabsTrigger>)}
          </TabsList>
          <DropdownMenu><DropdownMenuTrigger aria-label="More navigation" className={`grid h-12 w-12 shrink-0 place-items-center border-b-2 bg-transparent text-[#68716a] transition hover:bg-[#f2f5f1] hover:text-[#315d43] sm:h-11 sm:w-11 ${view==="history"?"border-[#315d43] text-[#244832]":"border-transparent"}`}><MoreHorizontal size={22}/></DropdownMenuTrigger><DropdownMenuContent align="end" className="min-w-40 bg-white"><DropdownMenuItem onClick={()=>setView("history")}><HistoryIcon/>History</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
