    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
          ...
          <div className="p-6 sm:p-9 lg:p-12">
            <div className="border-b border-[#e8e3da] pb-8">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex shrink-0 items-center justify-between gap-3 rounded-2xl bg-[#f2ecdf] p-3"><span className="font-medium">Cooking for</span><div className="flex items-center gap-1"><Button size="icon" variant="ghost" className="size-11 rounded-full sm:size-8" onClick={()=>changeServings(activeRecipe.id,-1)}><Minus size={17} className="sm:hidden"/><Minus size={15} className="hidden sm:block"/></Button><strong className="min-w-20 text-center">{servings[activeRecipe.id]||4} people</strong><Button size="icon" variant="ghost" className="size-11 rounded-full sm:size-8" onClick={()=>changeServings(activeRecipe.id,1)}><Plus size={17} className="sm:hidden"/><Plus size={15} className="hidden sm:block"/></Button></div></div>
              </div>
            </div>
      ...
      {view!=="recipes"&&<header className="relative -mx-4 -mt-6 mb-3 overflow-hidden rounded-none sm:mx-0 sm:mt-0 sm:rounded-[1.75rem]">
        ...
        <div className="relative flex items-stretch gap-2 px-5 py-3 sm:px-8">
          ...
          <TabsList className="flex !h-11 flex-1 items-stretch gap-0 overflow-hidden rounded-lg border-0 bg-white/15 p-0 shadow-none backdrop-blur-sm sm:flex-none">
            {nav.map(({value,label,icon:Icon})=><TabsTrigger key={value} value={value} className="relative flex h-full flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-none border-0 bg-transparent px-2 text-xs font-semibold text-white/80 shadow-none transition first:rounded-l-md last:rounded-r-md hover:bg-white/10 hover:text-white data-[state=active]:bg-[#257F4B] data-[state=active]:text-white data-[state=active]:shadow-none sm:px-5 sm:text-sm"><Icon size={16} className="hidden shrink-0 sm:block"/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className="grid size-[30px] shrink-0 place-items-center rounded-full bg-white/20 text-xs font-bold leading-none text-white">{selected.length}</b>}</TabsTrigger>)}
          </TabsList>
        </div>
      </header>}
        {view==="recipes"
          ? <div className="mb-4 flex items-center justify-between gap-2 pb-2 pt-4 sm:pt-3">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={()=>setOpen(true)} variant="outline" aria-label="Add a recipe" className="h-11 shrink-0 rounded-lg border-[#d9d5cc] bg-white px-4 text-sm font-semibold text-[#45644e] shadow-none transition-colors hover:bg-[#257F4B] hover:text-white sm:h-9"><Plus size={18}/>Add recipe</Button>
                {searchOpen
