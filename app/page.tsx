    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      {view!=="recipes"&&<header className="relative -mx-4 -mt-6 mb-3 overflow-hidden rounded-none sm:mx-0 sm:mt-0 sm:rounded-[1.75rem]">
        ...
        <div className="relative flex flex-col gap-4 px-5 py-6 text-white sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-8">
          ...
          <Button type="button" onClick={()=>goAway("recipes")} className="absolute right-4 top-4 z-10 flex size-20 shrink-0 flex-col items-center justify-center gap-1 rounded-full border-0 bg-[#FF7060] p-0 text-sm font-bold text-white shadow-lg transition-colors hover:bg-[#ff5c48] sm:size-28 sm:gap-1.5"><BookOpen size={18} className="sm:hidden"/><BookOpen size={26} className="hidden sm:block"/><span>Recipes</span></Button>
        </div>
        <div className="relative flex items-center gap-2 bg-[#1b2f22] px-5 py-3 sm:px-8">
          <Select value={view==="history"?"history":String(weekOffset)} onValueChange={value=>{if(value==="history"){goAway("history")}else{setWeekOffset(Number(value) as 0|1);if(view==="history")setView(preRecipesView)}}}><SelectTrigger aria-label="Choose planning week" className="data-[size=default]:h-11 min-w-0 shrink rounded-lg border-0 bg-[#257F4B] px-3 text-sm font-semibold text-white shadow-none [&_svg]:text-white sm:shrink-0">{view==="history"?<HistoryIcon size={16} className="shrink-0"/>:<CalendarDays size={16} className="shrink-0"/>}<SelectValue className="truncate">{view==="history"?"History":<><span className="sm:hidden">{weekLabelShort}</span><span className="hidden sm:inline">{weekLabel}</span></>}</SelectValue></SelectTrigger><SelectContent position="popper" sideOffset={4} align="start" className="bg-white">{[0,1].map(offset=><SelectItem key={offset} value={String(offset)}>{offset===0?"This week":"Next week"} · {weekRange(offset as 0|1)}</SelectItem>)}<SelectItem value="history">History</SelectItem></SelectContent></Select>
          <TabsList className="flex h-11 flex-1 items-stretch gap-1 overflow-hidden rounded-lg border-0 bg-[#257F4B] p-0 shadow-none sm:flex-none">
            {nav.map(({value,label,icon:Icon})=><TabsTrigger key={value} value={value} className="relative flex h-full flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-none border-0 bg-transparent px-2 text-xs font-semibold text-white/80 shadow-none transition first:rounded-l-md last:rounded-r-md hover:bg-white/10 hover:text-white data-[state=active]:bg-black/20 data-[state=active]:text-white data-[state=active]:shadow-none sm:px-5 sm:text-sm"><Icon size={16} className="shrink-0"/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className="grid size-[30px] shrink-0 place-items-center rounded-full bg-white/20 text-xs font-bold leading-none text-white">{selected.length}</b>}</TabsTrigger>)}
          </TabsList>
        </div>
      </header>}
        {view==="recipes"
