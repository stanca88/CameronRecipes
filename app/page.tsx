                        ...
                        <section key={cat} className="min-w-0 w-full overflow-hidden rounded-2xl border border-[#ddd4c3] bg-[#fffdf8] shadow-[0_2px_10px_rgba(45,61,50,.04)]">
                          ...
                          <div id={`cat-${cat}`} className={collapsed[cat]?"hidden p-2":"p-2"}>
                            {items.map(i=>{const key=`${i.name.toLowerCase()}|${i.unit.toLowerCase()}|${i.category}`;const done=syncedChecked.includes(key);const phrase=formatIngredientPhrase(i);return (
                              <label key={key} className={`flex min-w-0 cursor-pointer items-start gap-3 rounded-xl px-2 py-2.5 ${done?"text-[#9a9f9b] line-through":"hover:bg-[#f5f0e6]"}`}>
                                <Checkbox className="mt-1 size-5 shrink-0 sm:size-4" checked={done} onCheckedChange={()=>toggleShoppingItemSync(key,!done)}/>
                                <span className="min-w-0 flex-1 text-lg leading-relaxed text-[#1f3529] sm:text-base">{phrase}</span>
                              </label>
                            )})}
                          </div>
                        </section>
