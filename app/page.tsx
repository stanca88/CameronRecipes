...
import { ArrowLeft, BookOpen, CalendarDays, Check, ChefHat, ChevronDown, ChevronUp, History as HistoryIcon, Minus, MoreHorizontal, Plus, Search, ShoppingBasket, Trash2, X } from "lucide-react";
  ...
  return <main className="min-h-screen bg-[#faf9f5] text-[#1f3529]" style={{scrollbarGutter:'stable'}}>
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <Tabs value={view} onValueChange={setView}>
        <div className="mb-4 flex items-center justify-between gap-2 border-b border-[#dedbd2] pb-4 pt-4 sm:pt-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button type="button" variant="ghost" aria-label="View history" onClick={()=>setView("history")} className={`size-11 rounded-lg p-0 shadow-none transition sm:size-9 ${view==="history"?"bg-[#315d43] text-white hover:bg-[#274d37]":"text-[#45644e] hover:bg-[#f2f5f1]"}`}><HistoryIcon size={18}/></Button>
            <Button type="button" onClick={()=>setView("recipes")} className={`h-11 rounded-lg px-4 text-sm font-semibold shadow-none transition sm:h-9 ${view==="recipes"?"bg-[#315d43] text-white hover:bg-[#274d37]":"border border-[#d9d5cc] bg-white text-[#244832] hover:bg-[#f2f5f1]"}`}><BookOpen size={18}/>Recipes</Button>
          </div>
        </div>
        ...
        <div className="sticky top-0 z-30 mb-4 bg-[#faf9f5] pb-1.5 sm:static sm:mb-0 sm:bg-transparent sm:pb-0">
          <div className="flex items-center gap-2 overflow-hidden rounded-2xl border border-[#dedbd2] bg-white shadow-sm sm:rounded-none sm:border-0 sm:border-b sm:bg-transparent sm:shadow-none">
            <TabsList className="!h-auto grid flex-1 grid-cols-2 gap-1 rounded-none bg-transparent p-0 shadow-none sm:flex sm:flex-none sm:items-center sm:justify-start sm:rounded-none">
        ...
        <TabsContent value="recipes">
          <div className="mb-4 flex items-center justify-between gap-2">
            ...
            <Button onClick={()=>setOpen(true)} aria-label="Add a recipe" className="h-11 shrink-0 rounded-lg bg-[#315d43] px-4 text-sm font-semibold text-white shadow-none transition-colors hover:bg-[#274d37] sm:h-9"><Plus size={18}/>Add recipe</Button>
          </div>
