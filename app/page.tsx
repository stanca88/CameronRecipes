      ...
      <Tabs value={view} onValueChange={changeView}>
        {view==="recipes"
          ...
          : <div className="mb-4 flex items-center gap-2 pb-2 pt-4 sm:pt-3">
              <Select value={view==="history"?"history":String(weekOffset)} onValueChange={value=>{if(value==="history"){goAway("history")}else{setWeekOffset(Number(value) as 0|1);if(view==="history")setView(preRecipesView)}}}><SelectTrigger aria-label="Choose planning week" className="data-[size=default]:h-11 min-w-0 shrink rounded-lg border-[#d9d5cc] bg-white px-3 text-sm font-semibold text-[#244832] shadow-none sm:data-[size=default]:h-9 sm:shrink-0">{view==="history"?<HistoryIcon size={16} className="shrink-0"/>:<CalendarDays size={16} className="shrink-0"/>}<SelectValue className="truncate">{view==="history"?"History":<><span className="sm:hidden">{weekLabelShort}</span><span className="hidden sm:inline">{weekLabel}</span></>}</SelectValue></SelectTrigger><SelectContent position="popper" sideOffset={4} align="start" className="bg-white"><SelectItem value="0">This week · {weekRange(0)}</SelectItem><SelectItem value="1">Next week · {weekRange(1)}</SelectItem><SelectItem value="history">History</SelectItem></SelectContent></Select>
              <TabsList className="!h-auto flex flex-1 items-center gap-1 overflow-hidden rounded-lg border border-[#d9d5cc] bg-white p-1 shadow-none sm:flex-none sm:gap-2 sm:rounded-none sm:border-0 sm:border-b sm:bg-transparent sm:p-0 sm:shadow-none">
