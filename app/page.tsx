      ...
      {view!=="recipes"&&<header className="relative -mx-4 -mt-6 mb-3 overflow-hidden rounded-none sm:mx-0 sm:mt-0 sm:rounded-[1.75rem]">
        ...
        <div className="relative flex items-center gap-2 border-t border-white/15 bg-black/10 px-5 py-3 backdrop-blur-sm sm:px-8">
          ...
          <TabsList className="flex h-12 flex-1 items-stretch gap-1 overflow-hidden rounded-lg border border-white/40 bg-white/15 p-0 shadow-none backdrop-blur-sm sm:!h-auto sm:flex-none sm:items-center sm:gap-2 sm:rounded-none sm:border-0 sm:border-b-0 sm:bg-transparent sm:p-0 sm:shadow-none">
            {nav.map(({value,label,icon:Icon})=><TabsTrigger key={value} value={value} className="relative flex h-full flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-none border-0 bg-transparent px-2 text-xs font-semibold text-white/80 shadow-none transition first:rounded-l-md last:rounded-r-md hover:bg-white/10 hover:text-white data-[state=active]:bg-white data-[state=active]:text-[#244832] data-[state=active]:shadow-none sm:h-9 sm:flex-none sm:gap-2 sm:rounded-none sm:border-b-2 sm:border-transparent sm:px-5 sm:text-sm sm:first:rounded-none sm:last:rounded-none sm:data-[state=active]:border-white sm:data-[state=active]:bg-transparent sm:data-[state=active]:text-white sm:data-[state=active]:shadow-none"><Icon size={16} className="hidden shrink-0 sm:block"/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none ${view==="plan"?"bg-[#315d43]/20 text-[#244832]":"bg-white/25 text-white"} sm:bg-white/25 sm:text-white`}>{selected.length}</b>}</TabsTrigger>)}
          </TabsList>
        </div>
      </header>}
        {view==="recipes"
