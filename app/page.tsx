...
export default function Home() {
  ...
  const [plans,setPlans]=useState<Record<string,{selected:string[];servings:Record<string,number>;checked:string[]}>>({[weekKey(0)]:{selected:["pasta","chicken"],servings:{pasta:4,chicken:4},checked:[]}});
  ...
  const activePlan=plans[activeWeekKey]||{selected:[],servings:{},checked:[]};
  const selected=activePlan.selected;
  const servings=activePlan.servings;
  const checked=activePlan.checked;
  const setSelected=(update:(current:string[])=>string[])=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[]};return{...all,[activeWeekKey]:{...plan,selected:update(plan.selected)}}});
  const setServings=(update:(current:Record<string,number>)=>Record<string,number>)=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[]};return{...all,[activeWeekKey]:{...plan,servings:update(plan.servings)}}});
  const setChecked=(update:(current:string[])=>string[])=>setPlans(all=>{const plan=all[activeWeekKey]||{selected:[],servings:{},checked:[]};return{...all,[activeWeekKey]:{...plan,checked:update(plan.checked)}}});
  const weekLabel=weekRange(weekOffset);

  useEffect(()=>{try{const raw=localStorage.getItem("cameron-family-table");if(raw){const s=JSON.parse(raw);setRecipes(s.recipes||starterRecipes);setHistory(s.history||[]);setPlans(s.plans||{[weekKey(0)]:{selected:s.selected||[],servings:s.servings||Object.fromEntries((s.selected||[]).map((id:string)=>[id,4])),checked:s.checked||[]}})}}finally{setLoaded(true)}},[]);
  ...
  const deleteRecipe=(id:string)=>{setRecipes(v=>v.filter(recipe=>recipe.id!==id));setPlans(all=>Object.fromEntries(Object.entries(all).map(([key,plan])=>{const nextServings={...plan.servings};delete nextServings[id];return[key,{...plan,selected:plan.selected.filter(recipeId=>recipeId!==id),servings:nextServings}]})));setActiveRecipe(null)};
  ...
  return <main className="min-h-screen bg-[#faf9f5] text-[#1f3529]">
    <header className="border-b border-[#e5e1d8] bg-[#fdfcf9]/95"><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3.5 sm:px-6">
      ...
      <Dialog open={open} onOpenChange={value=>{setOpen(value);if(!value)resetAdd()}}><DialogTrigger aria-label="Add a family recipe" className="inline-flex h-10 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-[#315d43] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#274d37] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315d43]/35 sm:px-5"><Plus size={18}/><span>Add recipe</span></DialogTrigger><DialogContent className="bg-[#fffdf8]"><DialogHeader><DialogTitle>{addMode==="url"?"Add from a recipe link":addMode==="review"?"Review imported recipe":"Add a recipe manually"}</DialogTitle></DialogHeader>
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <section className="relative mb-8 min-h-[370px] overflow-hidden rounded-[1.75rem] sm:min-h-[440px]">
        ...
        <div className="relative flex min-h-[370px] max-w-2xl flex-col justify-center px-6 py-10 text-white sm:min-h-[440px] sm:px-10 lg:px-12"><div className="max-w-xl rounded-2xl bg-[#13271d]/75 p-6 shadow-sm backdrop-blur-sm sm:p-8">
          <p className="mb-3 text-xs font-bold uppercase tracking-[.2em] text-[#d5e7d8]">Cameron Family Table</p>
          <h2 className="max-w-xl font-serif text-3xl font-medium leading-tight sm:text-4xl lg:text-5xl">Good food, happy family.</h2>
          ...
          <div className="mt-6 flex flex-wrap gap-2"><span className="inline-flex items-center gap-2 rounded-lg bg-white/90 px-3 py-2 text-sm font-semibold text-[#244832]"><CalendarDays size={16}/>{weekLabel}</span><span className="inline-flex items-center rounded-lg bg-white/90 px-3 py-2 text-sm text-[#244832]"><strong className="mr-1">{selected.length}</strong> meals planned</span></div>
        </div></div>
      </section>
      ...
      <Tabs value={view} onValueChange={setView}>
        ...
        {view!=="history"&&<div className="mb-7"><Select value={String(weekOffset)} onValueChange={value=>setWeekOffset(Number(value) as 0|1)}><SelectTrigger className="h-11 w-full rounded-lg border-[#d9d5cc] bg-white px-4 font-semibold text-[#315d43] shadow-none sm:w-80"><CalendarDays size={17}/><SelectValue/></SelectTrigger><SelectContent className="bg-white"><SelectItem value="0">This week · {weekRange(0)}</SelectItem><SelectItem value="1">Next week · {weekRange(1)}</SelectItem></SelectContent></Select></div>}

        <TabsContent value="recipes">
          <div className="mb-5 flex items-end justify-between gap-4"><h2 className="font-serif text-2xl font-bold">Choose what sounds good for {weekLabel}.</h2><div className="relative hidden sm:block"><Search className="absolute left-3 top-2.5" size={18}/><Input className="w-64 rounded-lg bg-white pl-10" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search recipes"/></div></div>
        ...
        <TabsContent value="shop"><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-serif text-2xl font-bold">Shopping list</h2><p className="text-sm text-[#6d786f]">{weekLabel} · {grocery.length} items for {selected.length} recipes</p></div><Button variant="outline" className="w-fit rounded-full bg-[#fffdf8]" onClick={()=>setChecked(()=>[])}>Clear checks</Button></div><div className="grid min-w-0 gap-4 md:grid-cols-2">{categories.map(cat=><section key={cat} className="min-w-0 rounded-2xl border border-[#ddd4c3] bg-[#fffdf8] p-4 sm:p-5"><h3 className="mb-3 font-serif text-lg font-bold">{cat}</h3>{grocery.filter(i=>i.category===cat).map(i=>{const key=i.name+i.unit;const done=checked.includes(key);return <label key={key} className={`flex min-w-0 cursor-pointer items-center gap-2 rounded-xl px-1 py-2.5 sm:px-2 ${done?"text-[#9a9f9b] line-through":"hover:bg-[#f5f0e6]"}`}><Checkbox className="mr-1 shrink-0" checked={done} onCheckedChange={()=>setChecked(v=>done?v.filter(x=>x!==key):[...v,key])}/><strong className="shrink-0 whitespace-nowrap text-sm text-[#45644e]">{Math.round(i.amount*100)/100} {i.unit}</strong><span className="min-w-0 flex-1 break-words">{i.name}</span></label>})}</section>)}{!grocery.length&&<div className="md:col-span-2 rounded-3xl border border-dashed border-[#cfc5b2] bg-[#fffdf8]/60 p-10 text-center"><ShoppingBasket className="mx-auto mb-3 text-[#78907c]"/><p className="font-medium">Add meals to build your shopping list.</p></div>}</div></TabsContent>
