...
export default function Home() {
  ...
  const planRecipeGrid=(list:Recipe[])=><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{list.map((r,i)=><article key={r.id} onClick={()=>openRecipe(r)} className="group relative cursor-pointer overflow-hidden rounded-[1.75rem] border border-[#e1ddd3] bg-white transition hover:-translate-y-1 hover:shadow-lg"><div className="absolute right-3 top-3 z-10 opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100"><button onClick={event=>{event.stopPropagation();toggle(r.id)}} aria-label={`Remove ${r.title} from ${weekOffset===0?"this week":"next week"}`} className="grid size-11 place-items-center rounded-lg border border-white/40 bg-white/40 text-[#304439] shadow-sm backdrop-blur-sm transition-colors hover:border-[#d7a39a] hover:bg-white hover:text-[#a33f32] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315d43]/35 sm:size-10"><X size={20}/></button></div>{(r.image|| (r.id==="roasted-veg-bowl"?"/veg-bowl.jpg": r.id==="lemon-herb-chicken"?"/lemon-chicken.jpg": r.id==="beef-stew"?"/beef-stew.jpg":"")) ? <img src={r.image|| (r.id==="roasted-veg-bowl"?"/veg-bowl.jpg": r.id==="lemon-herb-chicken"?"/lemon-chicken.jpg": r.id==="beef-stew"?"/beef-stew.jpg":"")} alt={r.title} className="h-44 w-full object-cover"/> : <div className="grid h-36 place-items-center bg-[#edf2eb] text-5xl">{r.emoji}</div>}<div className="min-w-0 p-5"><h3 className="truncate font-serif text-xl font-bold" title={r.title}>{r.title}</h3><div className="mt-1.5 flex flex-wrap items-center gap-3" onClick={event=>event.stopPropagation()}><Input aria-label={`Day for ${r.title}`} value={days[r.id]||""} onChange={event=>setDay(r.id,event.target.value)} onBlur={()=>commitDay(r.id)} onKeyDown={event=>{if(event.key==="Enter"){event.stopPropagation();commitDay(r.id)}}} placeholder={`Meal ${i+1}`} className="h-6 w-24 shrink-0 rounded-none border-0 border-b border-transparent bg-transparent px-0 py-0 text-xs font-semibold uppercase tracking-wide text-[#FF7060] shadow-none placeholder:text-[#9a9f9b] hover:border-[#ddd4c3] focus-visible:border-[#FF7060] focus-visible:ring-0"/><Input aria-label={`Chef cooking ${r.title}`} value={chefs[r.id]||""} onChange={event=>setChef(r.id,event.target.value)} onKeyDown={event=>{if(event.key==="Enter")event.stopPropagation()}} placeholder="Chef" className="h-6 min-w-0 flex-1 rounded-none border-0 border-b border-transparent bg-transparent px-0 py-0 text-sm shadow-none placeholder:text-[#9a9f9b] hover:border-[#ddd4c3] focus-visible:border-[#9a735e] focus-visible:ring-0"/></div></div></article>)}</div>;
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      {view!=="recipes"&&<header className="relative -mx-4 -mt-6 mb-3 overflow-hidden rounded-none sm:mx-0 sm:mt-0 sm:rounded-[1.75rem]">
        ...
        <div className="relative flex flex-col gap-4 px-5 py-6 text-white sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-8">
          ...
          <Button type="button" onClick={()=>goAway("recipes")} className="absolute right-4 top-4 z-10 flex size-20 shrink-0 flex-col items-center justify-center gap-1 rounded-full border-0 bg-[#FF7060] p-0 text-sm font-bold text-white shadow-lg backdrop-blur-none transition-colors hover:bg-[#ff5c48] sm:static sm:h-auto sm:w-28 sm:flex-col sm:justify-center sm:gap-2 sm:self-stretch sm:rounded-2xl sm:border sm:border-white/40 sm:bg-white/15 sm:py-4 sm:shadow-none sm:backdrop-blur-sm sm:hover:bg-white sm:hover:text-[#244832]"><BookOpen size={18} className="sm:hidden"/><BookOpen size={28} className="hidden sm:block"/><span>Recipes</span></Button>
        </div>
        <div className="relative flex items-center gap-2 bg-[#1b2f22] px-5 py-3 sm:bg-transparent sm:border-t sm:border-white/15 sm:px-8 sm:py-3">
          <Select value={view==="history"?"history":String(weekOffset)} onValueChange={value=>{if(value==="history"){goAway("history")}else{setWeekOffset(Number(value) as 0|1);if(view==="history")setView(preRecipesView)}}}><SelectTrigger aria-label="Choose planning week" className="data-[size=default]:h-11 min-w-0 shrink rounded-lg border-0 bg-[#257F4B] px-3 text-sm font-semibold text-white shadow-none [&_svg]:text-white sm:shrink-0 sm:data-[size=default]:h-9 sm:border sm:border-white/30 sm:bg-white/10 sm:backdrop-blur-sm">{view==="history"?<HistoryIcon size={16} className="hidden shrink-0 sm:block"/>:<CalendarDays size={16} className="hidden shrink-0 sm:block"/>}<SelectValue className="truncate">{view==="history"?"History":<><span className="sm:hidden">{weekLabelShort}</span><span className="hidden sm:inline">{weekLabel}</span></>}</SelectValue></SelectTrigger><SelectContent position="popper" sideOffset={4} align="start" className="bg-white">{[0,1].map(offset=><SelectItem key={offset} value={String(offset)}>{offset===0?"This week":"Next week"} · {weekRange(offset as 0|1)}</SelectItem>)}<SelectItem value="history">History</SelectItem></SelectContent></Select>
          <TabsList className="flex h-11 flex-1 items-stretch gap-1 overflow-hidden rounded-lg border-0 bg-[#257F4B] p-0 shadow-none sm:!h-auto sm:flex-none sm:items-center sm:gap-2 sm:rounded-none sm:border-0 sm:border-b-0 sm:bg-transparent sm:p-0 sm:shadow-none">
            {nav.map(({value,label,icon:Icon})=><TabsTrigger key={value} value={value} className="relative flex h-full flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-none border-0 bg-transparent px-2 text-xs font-semibold text-white/80 shadow-none transition first:rounded-l-md last:rounded-r-md hover:bg-white/10 hover:text-white data-[state=active]:bg-black/20 data-[state=active]:text-white data-[state=active]:shadow-none sm:h-9 sm:flex-none sm:gap-2 sm:rounded-none sm:border-b-2 sm:border-transparent sm:px-5 sm:text-sm sm:first:rounded-none sm:last:rounded-none sm:data-[state=active]:border-white sm:data-[state=active]:bg-transparent sm:data-[state=active]:text-white sm:data-[state=active]:shadow-none"><Icon size={16} className="hidden shrink-0 sm:block"/><span>{label}</span>{value==="plan"&&selected.length>0&&<b className="grid size-[30px] shrink-0 place-items-center rounded-full bg-white/20 text-xs font-bold leading-none text-white">{selected.length}</b>}</TabsTrigger>)}
          </TabsList>
        </div>
      </header>}
        {view==="recipes"
