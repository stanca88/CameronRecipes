...
export default function Home() {
  ...
  const loadSharedRecipes=async()=>{try{const response=await fetch("/api/recipes");if(!response.ok)return false;const {recipes:shared}=await response.json();if(Array.isArray(shared)&&shared.length>0){const normalized=shared.map((r:any)=>({...r,ingredients:Array.isArray(r.ingredients)?r.ingredients.map((i:any)=>typeof i==="string"?parseIngredientLine(i):i):[],directions:Array.isArray(r.directions)?r.directions.filter((d:any):d is string=>typeof d==="string"):[]}));setRecipes(normalized);return true}setRecipes(starterRecipes);return false}catch(e){console.error("Failed to load shared recipes:",e);setRecipes(starterRecipes);return false}};
